    ...
    public function nuevo(&$parametros = null, $tipo = Usuario_model::SIAP, $language_text = null) {
        ...
        switch ($tipo) {
            case Usuario_model::SIAP:
                $salida = $this->nuevo_siap($parametros, $salida, $language_text);
                break;
            case Usuario_model::NO_SIAP:
                $salida = $this->nuevo_no_siap($parametros, $salida, $language_text);
                break;
            case Usuario_model::NO_IMSS://Externos
                $salida = $this->nuevo_no_imss($parametros, $salida, $language_text);
            break;
        }
        return $salida;
    }
    ...
    private function nuevo_no_imss(&$parametros, &$salida, $language_text) {
        ...
        if ($usuario_db) {
            $data['usuario'] = array(
                ...
                'apellido_paterno' => $parametros['paterno'],
                'apellido_materno' => $parametros['materno'],
                //'clave_categoria' => $categoria['clave_categoria'],
...
//                $datos['usuario']['id_docente'] = $docente['id_informacion_usuario'];
    ...
    public function get_usuarios($params = []) {
        ...
        $this->db->join('catalogo.delegaciones del', 'del.clave_delegacional = usuarios.clave_delegacional', 'left');
        $this->db->join('catalogo.categorias cat', 'cat.clave_categoria = usuarios.clave_categoria', 'left');
        $this->db->join('catalogo.departamento dep', 'dep.clave_departamental = usuarios.clave_departamental', 'left');
...
//                pr($params);
        ...
        if (isset($params['order_by']) && !isset($params['total'])) {
            if (!is_null($params['order_by'])){
                $this->db->order_by($params['order_by']['field'], $params['order_by']['order']);
            } else {
